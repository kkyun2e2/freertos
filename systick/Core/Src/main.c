...
uint8_t stateSwitch;
void callbackSystick() // 1ms 마다 자동호출 하는함수
{
  		...
  		if(taskB==0) // 100ms 주기
  			{
  				...
  				flagStateBuzzer ^= 1; // 익스클루시브 or 게이트
  				flagStateBuzzer == 0? enableBuzzer():disabeBuzzer();  // 조건:참:거짓
